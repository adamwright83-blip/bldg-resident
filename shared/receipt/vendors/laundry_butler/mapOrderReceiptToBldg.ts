 ...
 * Vendor-internal payload shape — not part of the public BLDG contract.
 */
import type { BldgReceiptLine, BldgReceiptViewModel } from "../../../receiptViewModel";
import { buildLbIntakeReceiptLines } from "./receiptLines";

/** Raw order receipt JSON from Laundry Butler API (server-to-server). */
export type LbOrderReceiptJson = {
...
export type MapLbOrderReceiptToBldgOptions = {
...
function linesFromApiLineItems(
  lineItems: NonNullable<LbOrderReceiptJson["lineItems"]>
): BldgReceiptLine[] {
...
function shouldUseIntakeLines(api: LbOrderReceiptJson): boolean {
  const st = serviceTypeNormalized(api.serviceType);
  if (
    st === "dry_cleaning" &&
    api.drycleanItemsJson &&
    typeof api.drycleanItemsJson === "object"
  )
    return true;
  if (
    st === "wash_fold" &&
...
function customerNameFrom(api: LbOrderReceiptJson): string {
...
function orderIdNumber(api: LbOrderReceiptJson): number {
...
function orderPlacedIso(api: LbOrderReceiptJson): string {
...
export function mapLbOrderReceiptJsonToBldgReceipt(
  api: LbOrderReceiptJson,
  opts: MapLbOrderReceiptToBldgOptions
  ...
  return {
    ...
    footerMessage:
      opts.footerMessage ?? "Thanks for your business. Have an amazing day!",
  };
}

export function orderPlacedDisplayIsMissing(api: LbOrderReceiptJson): boolean {
  const raw = api.timestamps?.createdAt ?? api.createdAt;
  if (!raw) return true;
